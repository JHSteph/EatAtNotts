...
class CustomerOrderDeliveryNotifications : Service() {//Gives notifications when changes applied to Customer Delivery Order

    private lateinit var dbref: DatabaseReference
    private lateinit var dbrefDeliveryPending:DatabaseReference
    private val channelID = "com.example.eatatnotts.CustomerOrder.channel1"
    ...
    override fun onCreate() {//Initialization, and detect changes from Firebase
        ...
        createNotificationChannel(channelID, "DeliveryChannel", "There is A New Delivery Order!")
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    for (userSnapshot in snapshot.children) {
                        ...
                        dbrefDeliveryPending = FirebaseDatabase.getInstance().getReference("${userName} Delivery")
    ...
    private fun displayNotification() {//Display notification to users
        ...
        val notification = NotificationCompat.Builder(this, channelID)
            .setContentTitle("Delivery Orders")
            .setContentText("You have a new delivery order!")
